...
Feature: Process order submission with WireCard SEPA payment method
  ...
  Scenario: Create new Payment Rule for WireCard integration with WireCard SEPA payment method
    Given I go to System/Payment Rules
    And I click "Create Payment Rule"
    And I check "Enabled"
    And I fill in "Name" with "WireCardSepa"
    And I fill in "Sort Order" with "1"
    And I select "€" from "Currency"
    And I select "WireCard - SEPA" from "Method"
    And I press "Add Method Button"
    And I save and close form
    Then I should see "Payment rule has been saved" flash message

  Scenario: Error from Backend when pay order with WireCard SEPA payment method
    ...
    And I fill "WireCardSEPAForm" with:
      | Account owner | test                          |
  ...
  Scenario: Successful order payment with WireCard SEPA payment method
    Given There are products in the system available for order
    And There is EUR currency in the system configuration
